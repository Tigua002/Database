...
app.post("/processes", (req, res) => {
    connection.query(
        "SELECT * FROM dataSpotUsers.processes WHERE owner = ?",
        [req.body.mail],
        (err, result) => {
            if (err) {
...
app.post("/checkToken", (req, res) => {
    connection.execute(
        "SELECT * FROM dataSpotUsers.sessions WHERE token = ?",
        [req.body.token],
        (err, result) => {
            if (err) {
...
app.post("/pull/server/:process", (req, res) => {
    connection.execute(
        `SELECT * FROM dataSpotUsers.processes WHERE Name = '${req.params.process}' `,
        (err, result) => {
            if (err) {
            ...
            exec(
                `cd ${bashPath.BashPath} \n bash ${bashPath.DisplayName}.sh`,
                (error, stdout, stderr) => {
                    if (error) {
                        console.error(
...
app.get("/status/server/:process", (req, res) => {
    ...
    pm2.connect((err) => {
        ...
        pm2.describe(appName, (err, processDescription) => {
            if (err) {
                res.status(500).send({
                    error: "Failed to get process description",
                });
                pm2.disconnect();
                return;
            }
...
app.post("/update/settings", (req, res) => {
    connection.execute(
        ...
        (err, results) => {
            ...
            fs.unlink(
                `../../../etc/nginx/sites-available/${req.body.OldDomain}`,
                (err) => {
                    if (err) {
                        console.error("Error deleting old domain file:", err);
                        return res
                            .status(500)
                            .send("Failed to delete old domain file");
                    }
                ...
                location / {
                    ...
                    fs.writeFile(
                        `../../../etc/nginx/sites-available/${req.body.Domain}`,
                        fileContents,
                        (err) => {
                            if (err) {
                                console.error(
                                ...
                                return res
                                    .status(500)
                                    .send("Failed to write new domain file");
                            }
                            exec(
...
app.post("/create/Server", (req, res) => {
    ...
    connection.execute(
        ...
        [
            ...
            req.body.Name,
            servName,
            `../DataspotServers/${req.body.Domain}`,
            req.body.Owner,
        ],
        (err, results) => {
            if (err) {
                ...
                `;
                    ...
                    fs.writeFile("./TestBash.sh", bashFile, (err) => {
                        if (err) {
                            ...
                            return res
                                .status(500)
                                .send("Failed to write bash script");
                        }
                        ...
                        exec("sh ./TestBash.sh", (err, stdout, stderr) => {
                            if (err) {
                                ...
                                return res
                                    .status(500)
                                    .send("Failed to execute bash script");
                            }

                            fs.writeFile(
                                `../DataspotServers/${req.body.Domain}/${lastPart}/.env`,
                                req.body.ENV,
                                (err) => {
                                    if (err) {
                                        console.error(
                            ...
                            `;
                                    fs.writeFile(
                                        `../DataspotServers/${req.body.Domain}/${req.body.Name}.sh`,
                                        gitBash,
                                        () => {
...
app.post("/delete/server/", (req, res) => {
    connection.query(
        `DELETE FROM dataSpotUsers.processes WHERE Name = ?`,
        [req.body.trueName],
        (err, result) => {
            if (err) {
                console.error("Database update error:", err);
...
app.post("/login/google", (req, res) => {
    ...
    try {
        ...
        if (!req.body.isNewUser) {
            ...
            connection.execute(
                "INSERT INTO dataSpotUsers.DataspotUsers (email, password, type) VALUES (?, ?, ?)",
                [req.body.username, md5(req.body.password), "google"],
                (err, result) => {
                    if (err) {
                        console.error("Database update error:", err);
                        return res.status(500).send("Database update failed");
...
app.post("/create/table", function (req, res) {
    ...
    // Use parameterized query to insert user
    connection.query(
        `CREATE TABLE ${req.body.name} (${string})`,
        function (err, result) {
            if (err) {
                console.error("Error creating user:", err);
                res.status(500).send(err);
...
app.post("/create/column", function (req, res) {
    ...
    // Use parameterized query to insert user
    connection.query(
        `ALTER TABLE ${req.body.table} ADD COLUMN ${name} ${tableType}`,
        function (err, result) {
            if (err) {
                console.error("Error creating user:", err);
                res.status(500).send(err);
...
app.post("/delete/row/", async function (req, res) {
    ...
    connection.execute(
        "SELECT base FROM dataSpotUsers.dataspotDatabases WHERE owner = ?",
        [user],
        (err, result) => {
            if (err) {
                res.status(500).send("Error checking token");
...
app.post("/FavDB", (req, res) => {
    connection.execute(
        ...
        (err, result) => {
            ...
            connection.execute(
                "SELECT * FROM dataSpotUsers.DataspotUsers WHERE email = ?",
                [username],
                (error, result) => {
                    if (
                        result[0].FavDB ==
...
app.post("/setFavDB", (req, res) => {
    connection.execute(
        ...
        (err, result) => {
            ...
            connection.execute(
                "UPDATE dataSpotUsers.DataspotUsers SET FavDB = ? WHERE email = ?",
                [`${req.body.db}.${req.body.tbl}`, username],
                (error, result) => {
                    if (error) {
    ...
    connection.execute(
        ...
        (err, result, fields) => {
            ...
            if (err) {
                ...
                return res
                    .status(500)
                    .send({ message: "Error saving file", success: false });
            }
            ...
            fs.rename(req.file.path, filePath, (err) => {
                ...
                connection.execute(
                    "INSERT INTO dataSpotUsers.Files (user, filepath, role, uploadDate, Filename, owner, parent, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    [
                        user,
                        customFilename,
                        "owner",
...
app.post("/uploadFolder", (req, res) => {
    ...
    connection.execute(
        ...
        (err, result, fields) => {
            let user = result[0].user;
            connection.execute(
                "INSERT INTO dataSpotUsers.Files (user, role, uploadDate, Filename, owner, parent, type) VALUES (?,  ?, ?, ?, ?, ?, ?)",
                [
                    user,
                    "owner",
                    DateName,
...
app.post("/FetchFiles", (req, res) => {
    connection.execute(
        "SELECT user FROM dataSpotUsers.sessions WHERE token = ?",
        [req.body.token],
        (error, result1, fields1) => {
            if (result1.length < 1) {
                res.status(500).send("Shady shi goin on");
                return;
            }
            let user = result1[0].user;
            connection.query(
                "Select Filename, uploadDate, filepath, type, owner FROM dataSpotUsers.Files WHERE user = ? AND parent = ?",
...
app.post("/changefile/delete", (req, res) => {
    connection.execute(`DELETE FROM dataSpotUsers.Files WHERE filepath = ?`, [
        req.body.file,
    ]);
...
app.post("/share/file", (req, res) => {
    ...
    connection.execute(
        "SELECT owner, uploadDate, Filename FROM dataSpotUsers.Files WHERE filepath = ?",
        [filePath],
        (err, result, fields) => {
            if (err) {
                ...
                console.log(result);
                res.status(500).send({
                    message: "Suspicious server error, contact admin",
                });
            }
            let file = result[0];
            connection.execute(
                "INSERT INTO dataSpotUsers.Files (user, filepath, role, uploadDate, Filename, owner) VALUES (?, ?, ?, ?, ?, ?)",
                [
                    user,
                    filePath,
                    "user",
